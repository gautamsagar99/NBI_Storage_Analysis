...
PARQUET_ROOT = Path("data/storage/parquet_snappy")
...
def main():
    ...
    print("Parquet file count:", len(pq_files))
    ...
    print("Missing Parquet files:", len(missing))
    for m in missing[:10]:
        print("  ", m)

    # 2) Total sizes
    print("Total raw size (MB):", dir_size(RAW_ROOT) / 1024 / 1024, "MB")
    print("Total Parquet size (MB):", dir_size(PARQUET_ROOT) / 1024 / 1024, "MB")
    ...
    if all_ok:
        print("All Parquet files validated successfully.")
    else:
        print("Some Parquet files had validation issues, investigate further.")
